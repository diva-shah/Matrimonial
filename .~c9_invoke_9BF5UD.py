...
def userProfile(username):
    ...
    idu = session['user_id']
    if len(check) == 1:
        return render_template("blocked.html",user=user, age=age, idu=idu, length=length, choice=choice, otherPerson=otherPerson)

    else:
        return render_template("user.html", user=user, age=age, idu=idu, length=length, choice=choice, otherPerson=otherPerson)
...
def register():
    ...
    if request.method == "POST":
        ...
        dob = db.execute("SELECT dob FROM users WHERE username = ?", request.form.get("username"))
        if calcAge(dob[0]['dob']) < 18:
            return apology("You must be at least 18 years to be on this website!")
            db.execute("DELETE FROM users WHERE userID = (SELECT userID FROM users WHERE username = ?)", request.form.get("username"))
        information = db.execute("SELECT * FROM users WHERE username = ?", request.form.get("username"))
        if check_password_hash(information[0]["hash"], request.form.get("password")) in db.execute("SELECT hash FROM users"):
            return apology("Password Taken")
...
def liked(username):
    ...
    length = len(plikes2)
    return render_template("user.html", user=user, age=age, length=length)
...
def myLikes():
    ...
    ageOfOthers=[]
    info = db.execute("SELECT * FROM users WHERE userID = ?", session["user_id"])
    liked = db.execute("SELECT * FROM profileLikes WHERE fromMe = ?", info[0]['username'])
    length = len(liked)
    ...
    likedMe = db.execute("SELECT * FROM profileLikes WHERE toUser = ?", info[0]['username'])
    length2 = len(likedMe)
...
def unliked(username):
    ...
    return render_template("user.html", user=user, age=age, length = 0)
